<template>
  <div class="menubox">
    <div v-for="(item, index) in menuItems" :key="index"
      :class="'menu_banner' + (index !== 0 ? ' menu_banner_margin' : '')">
      <div :class="'menu_item' + getParentMenuCls(index)" @click="onMenuItemClick(index)">
        <div class="menu_name_parent">{{ item.name }}</div>
      </div>
      <div v-for="(child, cIndex) in item.childs" :key="cIndex"
        :class="'menu_child_item' + getChildMenuCls(index, cIndex)" @click="onMenuChildClick(index, cIndex)">
        {{ child.name }}
      </div>
    </div>
